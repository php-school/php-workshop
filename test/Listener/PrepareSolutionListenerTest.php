...
class PrepareSolutionListenerTest extends TestCase
{
    ...
    public function testIfSolutionRequiresComposerButVendorDirExistsNothingIsDone(): void
    {
        ...
        mkdir(sprintf('%s/vendor', $context->referenceWorkingDirectory));
        $this->assertFileExists(sprintf('%s/vendor', $context->referenceWorkingDirectory));
        ...
        $this->assertFileDoesNotExist(sprintf('%s/composer.lock', $context->referenceWorkingDirectory));
    }
    ...
    public function testIfSolutionRequiresComposerComposerInstallIsExecuted(): void
    {
        ...
        $this->assertFileDoesNotExist(sprintf('%s/vendor', $context->referenceWorkingDirectory));
        file_put_contents(sprintf('%s/composer.json', $context->referenceWorkingDirectory), json_encode([
            'require' => [
                'phpunit/phpunit' => '~5.0'
            ],
        ]));
